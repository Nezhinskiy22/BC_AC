document.addEventListener("DOMContentLoaded", () => {
    ...
    let getProductsAvailability = (item) => {
    ...
    let renderProductAvailiability = (item, data) => {
...
let pageLoad = (a) => {
    let products = document.querySelectorAll(".product-item").length;
    ...
    dataLayer.push({
        "pageLoad": {
            ...
            "customer": "Guest",
            "products": products,
            "productsAvailable": products - a.length
        }
    })
}

const addButton = document.querySelectorAll(".product-item__add-btn");
const wishButton = document.querySelectorAll(".product-item__wishlist");
...
addButton.forEach(
    (elem, i) => {
        elem.addEventListener("click", (e) => {
            ...
            productAddToCart(itemName, productId, counter[0]);
            return counter[0]++;
        });
    });


let productAddToCart = (itemName, productId, counter) => {
    dataLayer.push({
        "productAddToCart": {
            "pageType": "PLP",
            "customer": "Guest",
...
wishButton.forEach(
    (elem, i) => {
        elem.addEventListener("click", () => {
            let productId = addButton[i].getAttribute('data-product-id');
            let itemName = productNames[i].textContent;
            productAddToWL(itemName, productId, counter[1]);
            return counter[1]++;
        });
    });

let productAddToWL = (itemName, productId, counter) => {
    dataLayer.push({
        "productAddToWL": {
            "pageType": "PLP",
            "customer": "Guest",
...
let colorSwatch = document.querySelectorAll(".filters__color-swatch");

colorSwatch.forEach(
    (elem, i) => {
        elem.addEventListener("click", () => {
            let colorName = colorSwatch[i].className.split(" ")[1];
            changeColor(colorName, counter[2]);
            return counter[2]++;
        });
    });

let changeColor = (colorName, counter) => {
    dataLayer.push({
        "changeColor": {
            "pageType": "PLP",
            "customer": "Guest",
